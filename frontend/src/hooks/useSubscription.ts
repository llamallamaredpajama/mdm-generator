...
// Feature access by tier (static lookup — hoisted to module scope)
const FEATURE_MAP = {
...
function determineSubscriptionTier(subscription: DocumentData | null): SubscriptionTier {
  ...
  const productId =
    subscription.items?.[0]?.price?.product?.id || subscription.items?.[0]?.plan?.product
...
export function useSubscription(): SubscriptionHookReturn {
  ...
  useEffect(() => {
    ...
    // Fetch real quota from whoAmI endpoint, falling back to static limits on failure
    async function fetchQuota(tier: SubscriptionTier) {
      try {
        const token = await user!.getIdToken()
        const info = await whoAmI(token)
        setRemainingGenerations(info.remaining)
      } catch {
        // whoAmI failed — fall back to static tier limit
      ...
      (snapshot: QuerySnapshot<DocumentData>) => {
        try {
          if (snapshot.empty) {
            ...
            setSubscription(null)
            if (prevTierRef.current !== 'free') {
              prevTierRef.current = 'free'
              void fetchQuota('free')
            }
          } else {
            ...
            const extractedProductId =
              itemPrice?.product?.id || data.items?.[0]?.plan?.product || null
...
export function useSubscriptionFeature(feature: FeatureKey): boolean {
