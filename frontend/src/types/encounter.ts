...
/**
 * Recommended order generated by Section 1 AI
 * Based on differential diagnoses and identified CDRs
 */
export interface RecommendedOrder {
  /** Test ID matching the test library (e.g., "troponin", "ecg") */
  testId: string
  /** Human-readable test name */
  testName: string
  /** Why this test is recommended, linked to differential or CDR */
  reasoning: string
  /** Priority: high (critical for emergent), medium (important), low (supplementary) */
...
export interface Section1Data {
  ...
  llmResponse?: {
    ...
    /** AI-recommended orders for S2 workup (based on differential + CDRs) */
 ...
 * In the redesigned flow, S2 is data-entry only — mdmPreview is not returned.
 * The field is kept optional for backward compatibility with old encounters.
 */
export interface Section2Response {
  /** Generated MDM preview (legacy — not returned in redesigned S2 flow) */
  mdmPreview?: MdmPreview
  /** Updated submission count for this section */
  submissionCount: number
  /** Whether the section is now locked */
