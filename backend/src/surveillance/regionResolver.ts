 ...
 * Provides state-level resolution without Firestore, covering all US ZIP ranges.
 */
export const ZIP_PREFIX_TO_STATE: Record<string, string> = {
  ...
  '911': 'CA', '912': 'CA', '913': 'CA', '914': 'CA', '915': 'CA', '916': 'CA', '917': 'CA', '918': 'CA',
...
export class RegionResolver {
  ...
  /**
   * Resolve state from ZIP prefix (first 3 digits).
   * Provides state-level resolution without Firestore.
   */
  resolveFromZipPrefix(zipCode: string): ResolvedRegion | null {
    const prefix = zipCode.slice(0, 3)
    const stateAbbrev = ZIP_PREFIX_TO_STATE[prefix]
