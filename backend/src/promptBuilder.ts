...
export async function buildPrompt(narrative: string): Promise<PromptParts> {
  ...
  const system = [
    ...
    '- Include the physician attestation: the treating physician provided the clinical input and has reviewed the output for accuracy and completeness.',
    '',
  ...
  const user = [
    ...
    '- Return a strict JSON object with keys: differential, data_reviewed_ordered, decision_making, risk, disposition, attestation.',
