...
/**
 * Section 2: Workup & Results (Data Entry Only)
 *
 * In the redesigned Build Mode, Section 2 is pure data entry — no AI processing.
 * The `content` field is optional (defaults to empty string) since structured data
 * fields are the primary input mechanism. The endpoint persists data to Firestore
 * without calling the LLM.
 *
 * Legacy behavior: Old clients may still send `content` with free-text narrative.
 * This is accepted and stored but NOT sent to an LLM.
 */
export const Section2RequestSchema = z.object({
  encounterId: z.string().min(1),
  content: z.string().max(SECTION2_MAX_CHARS).default(''),
  workingDiagnosis: z.string().optional(),
  userIdToken: z.string().min(10),
  // Structured data fields (primary input for redesigned S2)
...
/**
 * Recommended order item generated by Section 1 AI
 * Based on differential diagnoses and identified CDRs
 */
export const RecommendedOrderSchema = z.object({
  testId: z.string(),
  testName: z.string(),
  reasoning: z.string(),
...
/**
 * Section 1 Response: Worst-first differential diagnosis list + recommended orders
 */
export const Section1ResponseSchema = z.object({
  differential: z.array(DifferentialItemSchema),
  recommendedOrders: z.array(RecommendedOrderSchema).optional(),
