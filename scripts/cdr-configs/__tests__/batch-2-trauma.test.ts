...
describe('batch-2-trauma CDR definitions', () => {
  it(`exports exactly ${EXPECTED_CDR_COUNT} CDRs`, () => {
    expect(batch2TraumaCdrs).toHaveLength(EXPECTED_CDR_COUNT)
  })

  it('has unique IDs across all CDRs', () => {
    const ids = batch2TraumaCdrs.map((c) => c.id)
    const unique = new Set(ids)
    expect(unique.size).toBe(ids.length)
  })

  it('all IDs are snake_case', () => {
    for (const cdr of batch2TraumaCdrs) {
      expect(cdr.id).toMatch(SNAKE_CASE_RE)
    }
  })

  describe.each(batch2TraumaCdrs.map((c) => [c.id, c]))('%s', (_id, cdr) => {
      ...
      it.each(cdr.components.map((c) => [c.id, c]))(
        ...
        (_compId, comp) => {
          ...
          if (comp.type === 'select') {
            ...
            // Option values should be sequential starting from 0
