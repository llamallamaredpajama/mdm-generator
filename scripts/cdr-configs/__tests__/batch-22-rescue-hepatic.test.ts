...
describe('Batch 22 — Hepatic Rescue CDRs', () => {
  it('exports exactly 1 CDR definition', () => {
    expect(batch22RescueHepaticCdrs).toHaveLength(1)
  })
  ...
  it('all entries conform to CdrSeed type (required fields present)', () => {
    ...
    for (const cdr of batch22RescueHepaticCdrs) {
  ...
  it('all IDs are unique', () => {
    const ids = batch22RescueHepaticCdrs.map((c) => c.id)
