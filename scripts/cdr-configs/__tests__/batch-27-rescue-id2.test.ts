...
describe('batch-27-rescue-id2 CDR definitions', () => {
  it(`exports exactly ${EXPECTED_CDR_COUNT} CDRs`, () => {
    expect(batch27RescueId2Cdrs).toHaveLength(EXPECTED_CDR_COUNT)
  })

  it('has unique IDs across all CDRs', () => {
    const ids = batch27RescueId2Cdrs.map((c) => c.id)
    const unique = new Set(ids)
    expect(unique.size).toBe(ids.length)
  })

  it('all IDs are snake_case', () => {
    for (const cdr of batch27RescueId2Cdrs) {
  ...
  describe.each(batch27RescueId2Cdrs.map((c) => [c.id, c]))('%s', (_id, cdr) => {
