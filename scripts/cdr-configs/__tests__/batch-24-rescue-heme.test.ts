...
describe('Batch 24 — Hematology Rescue CDRs', () => {
  it('exports exactly 2 CDR definitions', () => {
    expect(batch24RescueHemeCdrs).toHaveLength(2)
  })
  ...
  it('all entries conform to CdrSeed type (required fields present)', () => {
    ...
    for (const cdr of batch24RescueHemeCdrs) {
  ...
  it('all IDs are unique', () => {
    const ids = batch24RescueHemeCdrs.map((c) => c.id)
  ...
  describe('individual CDR spot checks', () => {
    it('PLASMIC Score has 7 boolean components and sum scoring with max 7', () => {
      const plasmic = batch24RescueHemeCdrs.find((c) => c.id === 'plasmic_score')!
      ...
      // 5 lab components are user_input, 2 clinical are section1
      const userInputCount = plasmic.components.filter((c) => c.source === 'user_input').length
      const section1Count = plasmic.components.filter((c) => c.source === 'section1').length
      expect(userInputCount).toBe(5)
      expect(section1Count).toBe(2)
      // Scoring: 0-4 Low, 5 Intermediate, 6-7 High
      const ranges = [...plasmic.scoring.ranges].sort((a, b) => a.min - b.min)
      expect(ranges[0].min).toBe(0)
      expect(ranges[0].max).toBe(4)
      expect(ranges[1].min).toBe(5)
      expect(ranges[1].max).toBe(5)
      expect(ranges[2].min).toBe(6)
      expect(ranges[2].max).toBe(7)
    })

    it('ISTH DIC has 4 select components and sum scoring with max 8', () => {
      const dic = batch24RescueHemeCdrs.find((c) => c.id === 'isth_dic')!
      expect(dic).toBeDefined()
      expect(dic.components).toHaveLength(4)
      expect(dic.scoring.method).toBe('sum')
      expect(dic.category).toBe('HEMATOLOGY / COAGULATION')
      for (const comp of dic.components) {
        expect(comp.type).toBe('select')
        expect(comp.source).toBe('user_input')
      }
      // All 4 components are user_input (no section2 remaining)
      const section2Count = dic.components.filter((c) => c.source === 'section2').length
      expect(section2Count).toBe(0)
      // Scoring: 0-4 Low, 5-8 High
      const ranges = [...dic.scoring.ranges].sort((a, b) => a.min - b.min)
      expect(ranges[0].min).toBe(0)
      expect(ranges[0].max).toBe(4)
      expect(ranges[1].min).toBe(5)
      expect(ranges[1].max).toBe(8)
    })
  })
})
